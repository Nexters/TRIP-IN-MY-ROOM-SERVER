...
class UserController(@Autowired private val socialLoginService: SocialLoginService,
	...
	@PutMapping("")
	fun update(@AuthenticationPrincipal principal: IfUserPrincipal, @RequestBody updateIn: UserDto.UpdateIn): ResponseEntity<Any> {
		var userInfo: UserDto.BasicInfoOut = userService.updateByUserId(principal.getUserUUID(), updateIn)
		return ResponseEntity.status(HttpStatus.OK).body(userInfo)
	}
	
	@PostMapping("/token/refresh")
	fun reNewToken(@AuthenticationPrincipal principal: IfUserPrincipal, @RequestBody refreshTokenIn: UserDto.RefreshTokenIn): ResponseEntity<MutableMap<String, Any>> {
		var userId: UUID = principal.getUserUUID()
